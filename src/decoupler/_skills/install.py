"""Install the bundled decoupler Agent Skill for Claude Code.

This module exposes the ``decoupler-install-skills`` console script. It copies
the Agent Skill that ships inside the installed package into a location where
Claude Code discovers skills, so that a coding agent gains decoupler-specific
...
Run ``decoupler-install-skills`` to install, or ``--print-path`` to see where the
...
def install_skill(dest: Path | None = None, force: bool = False) -> Path:
    """Copy the bundled decoupler skill into a Claude Code skills directory.
    ...
    dest : pathlib.Path or None, optional
        ...
        ``~/.claude/skills/decoupler``. Default is None.
    force : bool, optional
...
def main(argv: list[str] | None = None) -> int:
    """Command-line entry point for ``decoupler-install-skills``.
