...
from decoupler._skills import install as install_mod
from decoupler._skills.install import (
    _bundled_skill_dir,
    _default_dest,
    install_skill,
)
...
def test_skill_frontmatter():
    ...
    assert fields.get("name") == "decoupler"
...
def test_install_missing_bundle_raises(tmp_path, monkeypatch):
    ...
    with pytest.raises(FileNotFoundError):
        install_skill(dest=tmp_path / "decoupler")
...
def test_install_excludes_checkpoints_and_pycache(tmp_path, monkeypatch):
    ...
    dest = install_skill(dest=tmp_path / "decoupler")
...
def test_default_dest_is_under_home_and_isolated(tmp_path, monkeypatch):
    ...
    assert result == tmp_path / ".claude" / "skills" / "decoupler"
...
def test_console_script_print_path():
    # The entry point is installed next to the running interpreter (bin/Scripts).
    exe = Path(sys.executable).parent / "decoupler-install-skills"
    if not exe.exists():
